...
WebAppWayland::WebAppWayland(const std::string& type,
                             ...
                             std::shared_ptr<ShellSurface> surface)
    : WebAppBase()
    , m_appWindow(0)
    , m_windowType(type)
    , m_lastSwappedTime(0)
...
void WebAppWayland::init(int width, int height, int surface_id)
{
    ...
    LOG_DEBUG("Initial size width %d, Height %d\n", width, height);

    if (m_surface) {
        LOG_DEBUG("surface %p\n", m_surface.get());

        Surface surface = m_surface->GetSurface();
	...
	case SurfaceType::BACKGROUND:
            ...
            break;
	case SurfaceType::PANEL: {
            Panel panel = m_surface->GetPanel();
            m_appWindow->SetAglPanel(panel.GetPanelEdgeData());
    ...
    LOG_DEBUG("Final size width %d, Height %d\n", width, height);
...
void WebAppWayland::onStageActivated()
{
    ...
    // executingStageActivated guard.
    static bool executingStageActivated = false;
    if (executingStageActivated)
        return;
